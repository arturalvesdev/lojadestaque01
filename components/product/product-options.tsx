...
export function ProductOptions({
  ...
  return (
    <div className="space-y-5 mb-6">
      {/* Tamanho — oculto quando único (auto-selecionado) */}
      {hasMultipleSizes && (
        <div>
          ...
          <p className="text-sm font-semibold text-foreground mb-3">
            {onColorChange ? (
              <>
                Cor
                {selectedColor && (
                  <span className="ml-1.5 font-normal text-primary">{selectedColor}</span>
                )}
              </>
            ) : (
              "Cor disponível"
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border text-sm font-medium transition-all ${
                    selectedColor === color.name
                      ...
                      : "border-border/40 bg-secondary/40 text-muted-foreground hover:border-primary/60 hover:bg-primary/5 hover:text-foreground"
                  }`}
                >
                  {color.hex && (
                    <span
                      className={`w-3.5 h-3.5 rounded-full border shrink-0 ${
                        selectedColor === color.name ? "border-primary/50" : "border-border/40"
                      }`}
